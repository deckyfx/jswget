...
function jswget(/*[Object configuration|String URL], [function success_callback]*/){
	...
	*/	
	...
	var protocol = getProtocol(configs);
	var sendoptions = createSendOption(protocol, configs);
	var request = createRequest(protocol, sendoptions, configs);
...
function parseConfig(){
	...
	configs.formdata = (configs.formdata===undefined)? null:QS.stringify(configs.formdata);    
	...
	configs.pipestream = (configs.pipestream===undefined)? null:configs.pipestream;
...
function createRequest(protocol, sendoptions, configs){
	...
	request.on('response', function(response) {		
		...
		if (responsecookie !== undefined) {
			...
			for (var i = 0; i < responsecookie.length; i++) {
				...
				configs.globalcookies[configs.cookiename][responseconfigs.cookiename] = responsecookieobject[i];
			}
		}
		...
		if (response.statusCode == 301 || response.statusCode == 302) {
			if (response.headers.location !== undefined && configs.follow && configs._followCount < configs.maxfollow) {
				...
				// Also change referer
				configs.headers.Referer = "http://data.smartfren.com/index.php/main_beta/index";				
		...
		if (configs.downloadmode) {
			...
			if (configs.downstream === undefined) {
				...
				response.on('end', function(){
					...
					sendoptions.method = configs.method;
			...
			response.on('end', function () {
				configs.onsuccess.apply(configs.scope, [result, request, response, configs]);
				configs.onend.apply(configs.scope, [request, configs])
			});
			return;
		}
	...
	request.on('connect', function(response, socket, head) {
		configs.onconnect.apply(configs.scope, [response, socket, head, request, configs]);
	});
	request.on('upgrade', function(response, socket, head) {
		configs.onupgrade.apply(configs.scope, [response, socket, head, request, configs]);
	});
