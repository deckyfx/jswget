function jswget(/*[Object options|String URL], [function success_callback]*/){
	/*
	 * the options are:
	...
	*/
	...
	options.query = (options.query===undefined)? "":QS.stringify(options.query);
	options.path = ((options.path===undefined)? "/":options.path) + ((options.query.length > 0)? ("?" + options.query):"");
	...
	options.onend = (options.onend===undefined)? new Function():options.onend;
	...
	var req = USEDLib.request(options, function(res) {
		...
		if (res.statusCode >= 400) {
			...
			options.onend.apply(options.scope, [req, options]);
			return;
		}
		...
		if (options.downloadmode) {
			var filesize = parseInt( (res.headers['content-length'] === undefined)? 0:res.headers['content-length'] );
